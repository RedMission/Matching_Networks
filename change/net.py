...
class AttentionalClassify(nn.Module):
    ...
    def forward(self, similarities, support_set_y):
        ...
        return preds
class DistanceNetwork(nn.Module): # 计算输入图像与支持集中每个支持图像之间的余弦距离。
    """
    ...
    def forward(self, support_set, input_image):
        """
        forward pass
        :param support_set:the embeddings of the support set images.shape[sequence_length,task_num,64]
        :param input_image: the embedding of the target image,shape[*sequence_length*,task_num,64]
        :return:shape[task_num,sequence_length]
        """
        eps = 1e-10
        similarities = [] # 存储每个支持图像与输入图像之间的余弦相似度
...
class MatchingNetwork(nn.Module):
    def __init__(self, arg):
        ...
        self.batch_size = arg.task_num
        self.keep_prob = arg.keep_prob
        self.num_channels = arg.imgc
        self.learning_rate = arg.lr
        self.num_classes_per_set = arg.n_way
        self.num_samples_per_class = arg.k_spt
        self.image_size = arg.imgsz
    ...
    def forward(self, support_set_images, support_set_y, target_image, target_y):
        ...
        :param target_image: shape[batch_size,*sequence_length*,num_channels,image_size,image_size]
        :param target_y:
        :return:
        """
        ...
        encoded_images.append(gen_encode)
        output = torch.stack(encoded_images, dim=0) # 将支持集向量和查询集向量组合到一起 k*n+q*n,task_num(5),1600

        # if we are considering full-context embeddings
        if self.f:
            output = self.lstm(output) # input:_，task_num，1600

        # get similarities between support set embeddings and target
        similarites = self.c(support_set=output[:-1], input_image=output[-1]) # similarites [task_num, n*k]

        # 独热编码
        support_set_y_one_hot = torch.nn.functional.one_hot(support_set_y, self.num_classes_per_set)
        support_set_y_one_hot = support_set_y_one_hot.float()
        # produce predictions for target probabilities
        preds = self.a(similarites, support_set_y=support_set_y_one_hot) # [task_num,way]
    ...
    def finetunning(self, support_set_images, support_set_y, target_image, target_y):
        encoded_images = []
        for i in np.arange(support_set_images.size(1)):
            gen_encode = self.g(support_set_images[:, i, :, :])
            encoded_images.append(gen_encode)
        ...
        gen_encode = self.g(target_image)
        encoded_images.append(gen_encode)
        output = torch.stack(encoded_images, dim=0)  # 将支持集向量和查询集向量组合到一起
