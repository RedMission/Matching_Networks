...
def sample_batch(data,batch_size,n_way,k_shot,k_query):
    """
    ...
    target_x = np.zeros((batch_size, k_query, data.shape[2], data.shape[3], data.shape[4]), np.float32)
    target_y = np.zeros((batch_size, k_query), np.int32)
    ...
    for i in range(batch_size):
        ...
        choose_samples = np.random.choice(data.shape[1], size=k_shot + k_query, replace=False)
        ...
        aaa = x_temp[:, :k_shot]
        support_set_x[i] = x_temp[:, :k_shot]  # 只保留一部分作为支持集
        support_set_y[i] = np.expand_dims(y_temp[:], axis=1)  # expand dimension
        target_x[i] = x_temp[choose_label, k_shot:]  # 剩下部分作为查询集
        target_y[i] = y_temp[choose_label]
    return support_set_x, support_set_y, target_x, target_y

def get_batch(dataset_name): # 调整way*shot
    """
...
if __name__ == '__main__':
    ...
    #######
    support_set_x, support_set_y, target_x, target_y = sample_batch(x_train, batch_size, n_way, k_shot, k_query)
